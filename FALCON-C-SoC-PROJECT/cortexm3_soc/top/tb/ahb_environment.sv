...
function void ahb_env::build_phase(uvm_phase phase);
  ...
  ahb_score=ahb_sram_sb::type_id::create("ahb_score",this);
  foreach(ahb_uvc_h[i])
  ahb_uvc_h[i] = ahb_uvc::type_id::create($sformatf("ahb_uvc_h[%0d]",i),this);
//  `uvm_info("xxxxxxxxxxxxx",$sformatf("HAS_SCOREBOARD=%0b",env_cfg.has_scoreboard),UVM_NONE)
  /** create scoreboard class */
  if(env_cfg.has_scoreboard) begin 
  
  ahb_sb_h = ahb_sb::type_id::create("ahb_sb_h",this);
  end
...
function void ahb_env::connect_phase(uvm_phase phase);
  ...
  sram_mon_h.item_collected_port.connect(ahb_score.item_collected_imp);
